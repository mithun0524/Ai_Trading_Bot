...
class UnifiedDatabaseManager:
    ...
    def __init__(self, db_path: str = None):
        self.db_path = db_path or "trading_platform.db"
        self.init_database()
            ...
            with self.get_connection() as conn:
                ...
                cursor.execute('''
                    INSERT INTO orders (
                        ...
                        price, stop_price, filled_quantity, filled_price, 
                        status, commission, timestamp, filled_timestamp
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    ...
                    order.status, getattr(order, 'commission', 0.0),
                    order.timestamp, getattr(order, 'filled_timestamp', None)
                ))
        ...
        try:
            with self.get_connection() as conn:
                ...
                ''', (
                    ...
                    getattr(order, 'filled_timestamp', None), 
                    order.order_id
                ))
    ...
    def store_live_quote(self, symbol: str, quote_data: Dict[str, Any]):
        """Store live market quote with improved error handling"""
        conn = None
        try:
            conn = self.get_connection()
            cursor = conn.cursor()
            
            # Use INSERT OR REPLACE to handle concurrent updates
            cursor.execute('''
            ...
            if "database is locked" in str(e):
                # Log but don't raise - this is expected under high concurrency
                logger.debug(f"Database temporarily locked for {symbol} - skipping update")
            else:
                logger.error(f"Database error storing quote for {symbol}: {e}")
            if conn:
                conn.rollback()
        except Exception as e:
    ...
    def store_position(self, position: PositionRecord):
        ...
        try:
            ...
            cursor.execute('''
                ...
                (symbol, quantity, avg_price, current_price, 
                 invested_amount, current_value, unrealized_pnl, realized_pnl, 
                 first_buy_date, last_update, is_active)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                position.symbol,
                position.quantity,
                ...
                position.entry_time or datetime.now(),
                position.last_update or datetime.now(),
                True
            ))
            conn.commit()
            logger.debug(f"Position stored: {position.symbol}")
        except Exception as e:
