...
class AlphaVantageSource(DataSource):
    """Alpha Vantage data source (backup)"""
...
class UnifiedLiveDataManager:
    ...
    def _setup_data_sources(self):
        ...
        # Backup: Alpha Vantage (if API key available)
        av_key = config.get('ALPHA_VANTAGE_API_KEY')
        if av_key:
            self.data_sources.append(AlphaVantageSource(av_key))
        
        logger.info(f"Configured {len(self.data_sources)} data sources")
